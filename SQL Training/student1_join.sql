...
CREATE TABLE Profile (
   ...
   city VARCHAR(50),
   mobileno VARCHAR(15),
   FOREIGN KEY (profileid) REFERENCES Student(studentid)
);
...
INSERT INTO Profile (profileid, city, mobileno) VALUES
(1, 'Pune', '9876543210'),
(2, 'Mumbai', '9823456780'),
(3, 'Nagpur', '9988776655'),
(4, 'Delhi', '9911223344'),
...
SELECT s.*
FROM Student s
JOIN Profile p ON s.studentid = p.profileid
WHERE p.city = 'Pune';
...
-- Find students with no profile.
select * from student where studentid not in (select profileid from profile);

SELECT s.*
FROM Student s
LEFT JOIN Profile p ON s.studentid = p.profileid
WHERE p.profileid IS NULL;

-- List students along with their mobile number and city.
